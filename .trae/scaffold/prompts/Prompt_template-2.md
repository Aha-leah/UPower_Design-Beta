...
*   **Single Theme Mode**: Do not mix Dark Mode (black backgrounds) and Light Mode sections within the same page unless explicitly requested for a high-contrast "night mode" section. The default is **Light/Earthy** throughout.
*   **Consistency**: All components (Features, Stats, etc.) must adhere to the global color palette defined below. Do not introduce arbitrary dark sections that break the visual flow.
